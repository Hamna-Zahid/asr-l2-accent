...
children.push(bullet([B("An audio-free adaptation study"), new TextRun(" in which weak (n-gram) and strong (neural) text-only LMs, applied via n-best rescoring on the frozen decoder, are evaluated through the same error taxonomy, with a per-first-language fairness analysis and a bootstrap significance test.")]));
children.push(bullet([B("An oracle-and-ladder analysis"), new TextRun(" that bounds audio-free rescoring: an oracle shows half the error is recoverable from the n-best, while an LM-strength ladder (4-gram to GPT-2-xl, plus a fine-tuned in-domain LM) shows the gain saturates at about a quarter of that ceiling - fixing the bound as acoustic, not LM-capacity.")]));
children.push(bullet([B("A cross-family generality test"), new TextRun(" (Whisper vs. wav2vec2-CTC) showing the effect is architecture-dependent and predicted by the framework: n-best diversity (oracle) bounds the gain and the decoder's internal LM determines whether a weak LM suffices.")]));
...
children.push(caption("Table 8: Phase B across two ASR families (L2-ARCTIC, 24 speakers, test split). The oracle head-room bounds the gain's magnitude; the internal LM determines whether a weak LM suffices. delta is relative WER at the dev-tuned lambda."));
children.push(img("fig_crossfamily.png", 470, 310));
children.push(caption("Figure 10: Cross-family contrast. On Whisper (internal LM, rich n-best) LM strength is decisive - the weak 4-gram hurts (+0.8%) while GPT-2 helps strongly (-8.0%). On wav2vec2 (CTC, no internal LM, thinner n-best: 26% vs 50% oracle) both LMs give a comparable, modest gain - LM strength barely matters. The effect is architecture-dependent, as the oracle bound and internal-LM distinction predict."));
