...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig_crossfamily.pdf}
\caption{Cross-family contrast. On Whisper (internal LM, rich n-best) LM strength is decisive---the weak 4-gram \emph{hurts} ($+0.8\%$) while GPT-2 helps strongly ($-8.0\%$). On wav2vec2 (CTC, no internal LM, thinner n-best: 26\% vs 50\% oracle) both LMs give a comparable, modest gain---LM strength barely matters. The effect is architecture-dependent, as the oracle bound and internal-LM distinction predict.}
